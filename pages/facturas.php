...
$query_facturaspacgrafica = "SELECT * FROM clientes ORDER BY clientes.Id_cliente DESC";
...
$varcliente_resultadobuscador = "xxx";
if (isset($_POST["dato"])) {
  $varcliente_resultadobuscador = $_POST["dato"];
}
mysql_select_db($database_sistemapacgrafica, $sistemapacgrafica);
$query_resultadobuscador = sprintf("SELECT * FROM clientes WHERE clientes.Identificacion LIKE %s OR clientes.Nombre LIKE  %s ", GetSQLValueString("%" . $varcliente_resultadobuscador . "%", "text"),GetSQLValueString("%" . $varcliente_resultadobuscador . "%", "text"));
$resultadobuscador = mysql_query($query_resultadobuscador, $sistemapacgrafica) or die(mysql_error());
$row_resultadobuscador = mysql_fetch_assoc($resultadobuscador);
$totalRows_resultadobuscador = mysql_num_rows($resultadobuscador);
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Todas las Facturas</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
  ...
  <div class="panel-body">
    ...
    <input id="dato" name="dato" type="text" class="form-control" placeholder="Buscar por Identificación o Nombres" required>
    <br>
...
<!-- /. fin buscador -->
   ...
   <!--resultado buscador-->
   <?php if ($totalRows_resultadobuscador > 0) { // Show if recordset not empty ?>
   <!--panel-->
   <div class="panel panel-info">
  <div class="panel-heading">Resultados Encontrados: <?php echo $totalRows_resultadobuscador ?> </div>
  <div class="panel-body">
  ...
  <!--panel-->
   <?php do { ?>
         ...
         <div class="panel-heading">
           <div class="row">
             ...
             <div class="col-xs-9 text-right">
               <div class="huge"><?php echo $row_resultadobuscador['Factura']; ?></div>
               <div><?php echo $row_resultadobuscador['Nombre']; ?></div>
               <div><?php echo $row_resultadobuscador['Fecha']; ?></div>
               <div>Estado: <strong style=" text-transform:uppercase;"><?php echo $row_facturaspacgrafica['Estado']; ?></strong></div>
               </div>
             </div>
           </div>
         <a href="detalles_factura.php?recordID=<?php echo $row_resultadobuscador['Id_cliente']; ?>">
     ...
     <?php } while ($row_resultadobuscador = mysql_fetch_assoc($resultadobuscador)); ?>
     <!--fin panel-->
      </div>
</div>
     <!--fin panel-->
...
mysql_free_result($resultadobuscador);
?>
